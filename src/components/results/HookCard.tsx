...
export function HookCard({ hook, sourceName, sourceType, sourceUrl }: HookCardProps) {
  const { favoriteHooks, toggleHookFavorite } = useApp();
  ...
  const [variation, setVariation] = useState<Hook | null>(null);
    ...
    <div className="card animate-slide-up cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <h3 className="font-semibold text-base leading-tight">{displayHook.headline}</h3>
          {variation && (
            <span className="text-xs text-[var(--ca-gold)] mt-1 block">Generated Variation</span>
          )}
        </div>
        <div className="flex items-center gap-2 ml-4">
          <div className={`score-badge ${getScoreClass(displayHook.viralityScore.total)}`}>
      ...
      {isExpanded && (
        <div className="space-y-6 pt-4 border-t border-[var(--ca-gray-dark)] animate-fade-in" onClick={(e) => e.stopPropagation()}>
          ...
          <div>
            ...
            <p className="text-sm text-[var(--ca-off-white)]">{displayHook.sourceClaim}</p>
          </div>
          ...
          <div>
            ...
            <div className="bg-[var(--ca-gray-dark)] rounded-lg p-4">
              <p className="text-sm text-[var(--ca-off-white)]">{displayHook.bridge}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-[var(--ca-off-white)]">{displayHook.bigIdeaSummary}</p>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                { label: 'Easy to Understand', value: displayHook.viralityScore.easyToUnderstand },
                { label: 'Emotional', value: displayHook.viralityScore.emotional },
                { label: 'Curiosity', value: displayHook.viralityScore.curiosityInducing },
                { label: 'Contrarian', value: displayHook.viralityScore.contrarian },
                { label: 'Provable', value: displayHook.viralityScore.provable },
              ].map(item => (
                <div
                  key={item.label}
